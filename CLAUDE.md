...
- **`lib/auth/auth-context.tsx`** — `AuthProvider` context wrapping the app; `useAuth()` hook for all auth state
...
- **`lib/csv/parser.ts`** — Parses Exportify-format CSV into `SpotifyTrack[]`
- **`hooks/usePlaylistTable.ts`** — Sorting, filtering, search, selection state for the playlist table
- **`components/Dashboard/`** — Main UI (~900 line root component); feature-grouped subcomponents
